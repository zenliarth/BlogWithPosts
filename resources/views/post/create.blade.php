...
@section('content')
    <div class="w-full bg-gray-300 shadow-sm border-b">
        <div class="w-10/12 h-20 mx-auto flex justify-between items-center text-gray-800">
            ...
            <div>
                <form class="flex gap-4" action="{{ route('logout') }}" method="post">
                    @csrf
    ...
    <div class="w-full pl-8 col-span-8 rounded text-gray-900">
        ...
        @endif
            <form class="py-4 flex flex-col" action="{{ route('posts.store') }}" method="POST">
            @csrf
                <label class="py-2 text-lg" for="title">Post title</label>
                <input class="rounded text-gray-600" type="text" name="title" id="title" value="{{ old('title') }}">
                <label class="py-2 text-lg" for="content">Content</label>
                <textarea class="rounded text-lg text-gray-600" name="content" id="content" cols="60" rows="10" value="{{ old('content') }}"></textarea>
                <button type="submit" class="mt-4 py-2 px-4 text-lg bg-blue-600 text-white rounded">Create New Post</button>
            </form>
